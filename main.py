...
def detect_dangerous_code(content):
    dangerous_patterns = [
        rb'\b(rm -fr|rm -r|if=/dev/null|if=/dev/zero|if=/dev/random|if=/dev/urandom|cmd erase|apparmor|setenforce|shred -f|ufw disable|iptables -F|setfacl|/proc/sysrq-trigger|:(){ :|:& };:)\b',
    ]
